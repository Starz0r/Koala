...
namespace Koala.ViewModels
{
    static class Logger
    {
        private static LoggingSession LoggingSession { get; } = new LoggingSession("mpv.MediaElement");
        ...
        {
            var loggingChannel = new LoggingChannel(string.IsNullOrWhiteSpace(name) ? "mpv" : name, null);
